...
def memoize_to_sqlite(filename: str = "cache.db"):
    ...
    """
    # Allow access from multiple threads; serialize access with a lock.
    db_conn = sqlite3.connect(filename, check_same_thread=False)
    ...
    def memoize(func):
        def wrapped(*args):
            ...
            # Check if the result is already cached (serialize DB access)
            with db_lock:
            ...
            # Compute the result without holding the DB lock (function may be slow)
            result = func(*args)

            # Cache the result (serialize DB access)
            try:
                with db_lock:
                    cursor = db_conn.cursor()
                    cursor.execute(
                ...
                # Swallow DB errors to avoid crashing the wrapped function
                pass
